 ...
 */
public class NestedScrollLinearLayout2 extends LinearLayout implements NestedScrollingParent {
    ...
    public void onNestedScroll(View target, int dxConsumed, int dyConsumed, int dxUnconsumed, int dyUnconsumed) {
        ...
        if (dyUnconsumed < 0) {
            if (getTranslationY() >= 0) {
                int consume = (int) Math.max(getTranslationY() - OFFSET, dyUnconsumed);
                setTranslationY(getTranslationY() - consume);
            }
        } else {
            if (getTranslationY() <= OFFSET) {
                int consume = (int) Math.min(dyUnconsumed, getTranslationY());
                setTranslationY(getTranslationY() - consume);
            }
        }
    }
